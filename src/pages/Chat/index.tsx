  ...
  return (
    <div className={`message-item ${isMe ? 'message-item-me' : 'message-item-other'}`}>
      ...
      >
        {/* 他人消息：左侧头像 */}
        {!isMe && (
          <Avatar src={avatar} icon={<UserOutlined />} size={32} style={{ marginRight: 8 }} />
        )}
        ...
        >
          {!isMe && <div style={{ fontSize: 12, color: '#999', marginBottom: 4 }}>{username}</div>}
          <div>{content}</div>
          {timestamp && (
            <div
              style={{
        ...
        {/* 自己的消息：右侧头像 */}
        {isMe && (
          <Avatar src={avatar} icon={<UserOutlined />} size={32} style={{ marginLeft: 8 }} />
        )}
      </div>
    </div>
  );
            ...
            >
              {chatRooms.length > 0 ? (
                ...
                <div style={{ padding: 16, textAlign: 'center' }}>
                  <Text type="secondary">{isConnected ? '暂无聊天室' : '请先连接WebSocket'}</Text>
                </div>
              )}
            </Card>
          </Col>
