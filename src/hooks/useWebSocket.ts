...
import { chatWebSocketManager } from '@/utils/websocket';
import type { User, ChatRoom, ChatMessage } from '@/utils/websocket';
...
export const useChatWebSocket = (): UseChatWebSocketReturn => {
            ...
            prevRooms.map((room) => {
              if (room.id === historyRoomId) {
                ...
                const newHistoryMessages = historyMessages.filter(
                  (msg: ChatMessage) => !existingMessageIds.includes(msg.id || ''),
                );
                return {
                  ...room,
  ...
  const loadHistoryMessages = useCallback(
    (roomId: string) => {
      ...
      // 设置加载状态
      setChatRooms((prevRooms) =>
        prevRooms.map((r) => (r.id === roomId ? { ...r, isLoadingHistory: true } : r)),
      );
      ...
      if (!success) {
        // 如果发送失败，重置加载状态
        setChatRooms((prevRooms) =>
          prevRooms.map((r) => (r.id === roomId ? { ...r, isLoadingHistory: false } : r)),
        );
      }
    },
    [chatRooms],
